...
await test('eight released migrations upgrade with existing tenant/customer/audit rows intact and repeat as no-op',{timeout:30000},async t=>{
  ...
  const before=(await owner.query('SELECT id,organization_id,franchise_id,name,phone_normalized,phone_display,address,version,created_at,updated_at FROM shipit.customers')).rows,roots=(await owner.query('SELECT * FROM shipit.franchises')).rows;
  assert.deepEqual(await db.migrate(),{applied:18});assert.deepEqual(await db.migrate(),{applied:0});
  assert.deepEqual((await owner.query('SELECT id,organization_id,franchise_id,name,phone_normalized,phone_display,address,version,created_at,updated_at FROM shipit.customers')).rows,before);assert.deepEqual((await owner.query('SELECT * FROM shipit.franchises')).rows,roots);
