...
await test('Issue 21 upgrades through booking retrieval without changing existing tenant/customer/audit rows; repeated install is a no-op',{timeout:30000},async t=>{
  ...
  const before=(await owner.query('SELECT * FROM shipit.franchises')).rows;assert.deepEqual(await db.migrate(),{applied:19});assert.deepEqual(await db.migrate(),{applied:0});assert.deepEqual((await owner.query('SELECT * FROM shipit.franchises')).rows,before);
...
await test('failed additive booking migration rolls back schema/ledger; retry and later forward repair preserve history',{timeout:30000},async t=>{
  ...
  assert.deepEqual(await db.migrate({dir:repaired}),{applied:19});
  await writeFile(join(repaired,'1791133200001-synthetic-forward-repair.cjs'),"exports.up=pgm=>pgm.sql('CREATE INDEX synthetic_booking_repair_idx ON shipit.bookings(confirmed_at)');");
