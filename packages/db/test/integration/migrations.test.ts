...
await test('fresh migrations persist a ledger, repeat as no-op and create tenancy, authentication and membership tables', { timeout: 20000 }, async (t) => {
  ...
  assert.deepEqual(await database.migrate(), { applied: 11 });
  ...
  assert.deepEqual(tables.rows, [...['audit_records','auth_challenges','auth_delivery_jobs','auth_identifiers','auth_rate_limits','auth_security_events','auth_sessions','auth_users'].map(name=>({schema:'shipit',name})),...['booking_audit_events','booking_commands','booking_obligations','bookings','customer_audit_events','customer_commands','customers','domain_events','franchises'].map(name=>({schema:'shipit',name})),
    ...
    { schema: 'shipit', name: 'onboarding_commands' }, { schema: 'shipit', name: 'organizations' }, { schema: 'shipit', name: 'parcels' }, ...['pricing_audit_events','pricing_cards','pricing_commands','pricing_quotes','pricing_rules','pricing_versions','tax_audit_events','tax_calculations','tax_cards','tax_commands','tax_intents','tax_resolutions','tax_versions'].map(name=>({schema:'shipit',name})), { schema: 'shipit_migrations', name: 'pgmigrations' }]);
});
...
await test('released Issue 10 infrastructure upgrades to tenancy and repeated migration preserves roots', { timeout: 20000 }, async (t) => {
  ...
  assert.deepEqual(await database.migrate(), { applied: 10 });
  ...
  assert.deepEqual(await migrationNames(database), [
    ...
    '1789045200000-memberships-authorization', '1789059600000-tenant-audit-ownership', '1789146000000-append-only-audit', '1789232400000-independent-onboarding', '1789318800000-tenant-private-customers', '1789405200000-versioned-pricing', '1789491600000-tax-proposals', '1789578000000-atomic-bookings',
  ]);
});
