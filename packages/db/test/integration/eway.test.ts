...
await test('e-way migration upgrades populated attachment-era main, rolls back failure and leaves unknown declarations absent',{timeout:30000},async t=>{
  ...
  assert.deepEqual(await db.migrate(),{applied:5});assert.deepEqual(await db.migrate(),{applied:0});await db.prepareEway();assert.deepEqual(await snapshot(),before);
  ...
  await writeFile(join(repair,'1790787600001-synthetic-forward-repair.cjs'),"exports.up=p=>p.sql('CREATE INDEX synthetic_eway_repair ON shipit.eway_records(organization_id,franchise_id,version)');exports.down=()=>{throw new Error('Forward only');};");
