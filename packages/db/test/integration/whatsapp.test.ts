...
await test('WhatsApp additive upgrade preserves populated #35 baseline and audit, rollback and retry',{timeout:30000},async t=>{
  ...
  assert.deepEqual(await snapshot(),before);assert.deepEqual(await db.migrate(),{applied:5});assert.deepEqual(await db.migrate(),{applied:0});assert.deepEqual(await snapshot(),before);
});
