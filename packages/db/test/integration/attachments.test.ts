...
await test('attachment upgrade preserves populated main, rolls back failed migration and repeats without backfill',{timeout:30000},async t=>{
 ...
 assert.deepEqual(await db.migrate(),{applied:1});assert.deepEqual(await db.migrate(),{applied:0});await db.prepareAttachments();assert.deepEqual(await snapshot(),before);
