...
export function inspectSource(file, source) {
  ...
  function visit(node) {
    ...
    !(file === 'apps/api/src/modules/security/jobs.ts' && node.expression.getText(tree) === 'tx' && ts.isCallExpression(node.parent) && ["'SELECT organization_id,franchise_id,intent_id FROM shipit.whatsapp_outbound_scope($1,$2,$3)'", "'SELECT shipit.whatsapp_receive($1,$2,$3)'", "'SELECT organization_id,franchise_id,inbox_id FROM shipit.whatsapp_consent_next()'", "'SELECT organization_id,franchise_id,inbox_id FROM shipit.whatsapp_inbox_next()'", "'SELECT organization_id,franchise_id FROM shipit.attachment_cleanup_scope($1)'", "'SELECT organization_id,franchise_id,challenge_id FROM shipit.delivery_challenge_cleanup_scope($1)'", "'SELECT organization_id,franchise_id FROM shipit.outbox_next_scope($1,$2,$3,$4)'", "'SELECT organization_id,franchise_id FROM shipit.outbox_job_scope($1)'", "'SELECT organization_id,franchise_id,fanout_id FROM shipit.route_delay_fanout_scope($1)'", "'SELECT shipit.notification_policy_activate($1,$2,$3)'", '"SET LOCAL transaction_timeout = \'25s\'"' ].includes(node.parent.arguments[0]?.getText(tree))) &&
        !((file.endsWith('/auth/webhook.ts') && node.expression.getText(tree) === 'r') ||
          (['apps/api/src/modules/deliveries/routes.ts', 'apps/api/src/modules/whatsapp/routes.ts', 'apps/api/src/modules/outbox/routes.ts', 'apps/api/src/modules/eway/routes.ts', 'apps/api/src/modules/attachments/routes.ts', 'apps/api/src/modules/receipts/routes.ts', 'apps/api/src/modules/payments/routes.ts', 'apps/api/src/modules/routes/routes.ts', 'apps/api/src/modules/lots/routes.ts', 'apps/api/src/modules/bookings/routes.ts', 'apps/api/src/modules/parcels/routes.ts', 'apps/api/src/modules/tax/routes.ts', 'apps/api/src/modules/audit/routes.ts', 'apps/api/src/modules/customers/routes.ts', 'apps/api/src/modules/pricing/routes.ts'].includes(file) && node.expression.getText(tree) === 'request' && !(ts.isCallExpression(node.parent) && node.parent.expression === node)))) report(node, 'raw query method bypasses scopedQuery');
      ...
      if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && scopedNames.has(node.expression.text)) {
        ...
        if (/shipit\.delivery_[a-z_]+\b/.test(text) && !text.includes('{{franchise:')) report(node, 'delivery SQL requires both organization and franchise ownership');
