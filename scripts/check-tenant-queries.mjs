    ...
    if (!rawSql.has(file)) {
      ...
      if (ts.isPropertyAccessExpression(node) && node.name.text === 'query' &&
        !((file.endsWith('/auth/webhook.ts') && node.expression.getText(tree) === 'r') ||
          (['apps/api/src/modules/tax/routes.ts', 'apps/api/src/modules/audit/routes.ts', 'apps/api/src/modules/customers/routes.ts', 'apps/api/src/modules/pricing/routes.ts'].includes(file) && node.expression.getText(tree) === 'request' && !(ts.isCallExpression(node.parent) && node.parent.expression === node)))) report(node, 'raw query method bypasses scopedQuery');
