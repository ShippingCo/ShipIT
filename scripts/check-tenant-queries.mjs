  ...
  function visit(node) {
    ...
    if (!rawSql.has(file)) {
      ...
      if (ts.isPropertyAccessExpression(node) && node.name.text === 'query' &&
        !(file === 'apps/api/src/modules/security/jobs.ts' && node.expression.getText(tree) === 'tx' && ts.isCallExpression(node.parent) && ["'SELECT shipit.whatsapp_receive($1,$2,$3)'", "'SELECT organization_id,franchise_id,inbox_id FROM shipit.whatsapp_inbox_next()'", "'SELECT organization_id,franchise_id FROM shipit.attachment_cleanup_scope($1)'", "'SELECT organization_id,franchise_id FROM shipit.outbox_next_scope($1,$2,$3,$4)'", "'SELECT organization_id,franchise_id FROM shipit.outbox_job_scope($1)'", '"SET LOCAL transaction_timeout = \'25s\'"' ].includes(node.parent.arguments[0]?.getText(tree))) &&
