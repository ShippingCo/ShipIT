...
type Area='workspace'|'packages'|'deliveries'|'lots'|'routes'|'eway'|'booking'|'receipts'|'settings';
...
export default function BusinessShell({context,select,controller}:{controller:ScopeController;context:OperatorContext;select:(id:string)=>void}){
 ...
 const segment=location.pathname.split('/').filter(Boolean)[1]??'';const area:Area=segment==='packages'||segment==='deliveries'||segment==='lots'||segment==='routes'||segment==='eway'||segment==='receipts'||segment==='settings'?segment:segment==='new-booking'||segment==='booking'?'booking':'workspace';
 const roles=current.roles,links=[
  ...
  {area:'deliveries' as const,to:'/business/deliveries',icon:'local_shipping',label:'Deliveries',show:can(roles,['delivery_agent','dispatcher','franchise_admin'])},
