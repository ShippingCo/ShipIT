...
import type { RouteBoundary } from './types.ts';
const uuid=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;

/** Opaque, authenticated and purpose-separated. A cursor is continuation state, never authority. */
export function routeCursorCodec(key:Buffer,now=()=>Date.now()) {
  const derived=createHash('sha256').update('shipit:route:cursor:v1\0').update(key).digest();
  return {
    encode(binding:string,boundary:RouteBoundary) {
      const iv=randomBytes(12),cipher=createCipheriv('aes-256-gcm',derived,iv);
      cipher.setAAD(Buffer.from('route:v1'));
      const plaintext=Buffer.from(JSON.stringify({binding,boundary,expires:now()+900_000}));
      const encrypted=Buffer.concat([cipher.update(plaintext),cipher.final()]);
      return Buffer.concat([iv,cipher.getAuthTag(),encrypted]).toString('base64url');
    },
    decode(token:string,binding:string):RouteBoundary {
      try {
        ...
        decipher.setAAD(Buffer.from('route:v1'));decipher.setAuthTag(data.subarray(12,28));
