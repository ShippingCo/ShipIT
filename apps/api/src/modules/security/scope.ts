...
export type PrivateAction = import('../lots/types.ts').LotAction | BookingAction | import('../parcels/types.ts').ParcelAction | import('../tax/types.ts').TaxAction | import('../pricing/types.ts').PricingAction | CustomerAction | ApprovedTenancyContext['action'] | 'memberships.read' | 'memberships.manage' |
...
export function issueTenantAccess(executor: QueryExecutor, input: PrivateContext, transaction = true): TenantAccess {
  ...
  if ((['bookings.create','parcels.create','customer.snapshot.read','bookings.audit','bookings.events'].includes(input.action) || input.action.startsWith('lots.') || input.action.startsWith('parcels.') || input.action.startsWith('tax.') || input.action.startsWith('pricing.') || ['customer.read','customer.list','customer.create','customer.update'].includes(input.action)) &&
...
export function scopedQuery<Row extends object = Record<string, unknown>>(
  ...
  const writes = command !== 'SELECT' || /\bshipit\.append_(?:tenancy_audit|security_denial|customer_audit|pricing_audit|booking_audit|lot_audit)\s*\(/i.test(sql);
