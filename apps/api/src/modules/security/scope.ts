...
export type PrivateAction = 'whatsapp.inbox.work' | 'whatsapp.read' | 'whatsapp.write' | import('../outbox/types.ts').OutboxAction | import('../eway/types.ts').EwayAction | import('../attachments/types.ts').AttachmentAction | import('../receipts/types.ts').ReceiptAction | import('../payments/types.ts').PaymentAction | import('../routes/types.ts').RouteAction | import('../lots/types.ts').LotAction | BookingAction | import('../parcels/types.ts').ParcelAction | import('../tax/types.ts').TaxAction | import('../pricing/types.ts').PricingAction | CustomerAction | ApprovedTenancyContext['action'] | 'memberships.read' | 'memberships.manage' |
...
export function issueTenantAccess(executor: QueryExecutor, input: PrivateContext, transaction = true): TenantAccess {
  ...
  if (input.action.startsWith('whatsapp.') && input.action!=='whatsapp.inbox.work' && (input.provenance !== 'membership' || input.actor.type !== 'user' || input.organizationWide || input.permittedFranchiseIds.length !== 1)) throw new HttpError('ACTION_FORBIDDEN');
...
export function scopedQuery<Row extends object = Record<string, unknown>>(
  ...
  const writes = command !== 'SELECT' || /\bshipit\.(?:outbox_(?:relay|claim|receipt|finish|redrive)|whatsapp_inbox_process|append_(?:payment_audit|tenancy_audit|security_denial|customer_audit|pricing_audit|booking_audit|lot_audit|route_audit))\s*\(/i.test(sql);
