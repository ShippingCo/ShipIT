...
export async function lockOrganization(tx: TenantAccess, organizationId: string, exclusive = false): Promise<Organization> {
  assertTenantAccess(tx, ['franchise.create', 'organization.profile.update', 'organization.lifecycle.manage', 'franchise.profile.update', 'franchise.lifecycle.manage']); assertOrganization(tx, organizationId);
  assertTenantAccess(tx);
  const result = await scopedQuery<OrganizationRow>(tx, ['franchise.create', 'organization.profile.update', 'organization.lifecycle.manage', 'franchise.profile.update', 'franchise.lifecycle.manage'], `SELECT ${organizationColumns} FROM shipit.organizations
...
export async function lockFranchise(tx: TenantAccess, scope: FranchiseScope, exclusive = false): Promise<Franchise> {
  assertTenantAccess(tx, ['franchise.profile.update', 'franchise.lifecycle.manage']); assertOrganization(tx, scope.organizationId); assertFranchises(tx, [scope.franchiseId]);
  assertTenantAccess(tx);
  const result = await scopedQuery<FranchiseRow>(tx, ['franchise.profile.update', 'franchise.lifecycle.manage'], `SELECT ${franchiseColumns} FROM shipit.franchises
