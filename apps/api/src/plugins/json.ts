...
export function registerJson(app: FastifyInstance) {
  ...
  app.addContentTypeParser('application/json', { parseAs: 'buffer', bodyLimit: JSON_BODY_LIMIT }, (request, body, done) => {
    try { done(null, parseStrictJson(new TextDecoder('utf-8', { fatal: true }).decode(body as Buffer), /\/(?:pricing|payments|eway|outbox|whatsapp)(?:\/|$)/.test(request.routeOptions.url??''))); }
