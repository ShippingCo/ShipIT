...
Issue #14 update: [ADR 0012](../adr/0012-membership-invitations-and-rbac.md) and the
[membership API](membership-authorization.md) implement fixed roles, scoped grants,
identity-bound invitations and live revocation. Product-wide resource projections remain
with #15 and general durable audit remains with #16.
...
| D02 | Complete role/action/resource/scope policy | [Authorization matrix](authorization-contract.md) with seven fixed roles, W41 lifecycle grants and W42 guarded org membership administration; unapproved actions denied | #3 / tenancy; #12 / lifecycle; #14 / memberships and grants | Contracts resolved; #15 must apply them to every product resource before exposure | #6, #15, #17, #42 | RESOLVED contract; #14 mechanics implemented for review; product-wide enforcement outstanding |
...
| D05 | Concrete lease duration, retry/backoff, concurrency/polling, fairness, poison/redrive and consumer deduplication persistence mechanics | [ADR 0023](../adr/0023-durable-outbox-worker.md) implements PostgreSQL leases, fair scheduling, receipts and W44 redrive; [ADR 0027](../adr/0027-durable-whatsapp-outbound.md) implements durable provider intents; [ADR 0028](../adr/0028-event-notification-automation.md) implements event policy/cause dedupe; [ADR 0029](../adr/0029-route-delay-notification-fanout.md) fixes the 20-item resumable delay fanout and W19 reminder cooldown | #35 / outbox; #39 / messaging; #40/#41 / consumer dedupe/fanout | Implemented boundaries precede downstream policies | #45 | RESOLVED for #35/#39/#40/#41; later provider-policy owners retain their own gates |
| D06 | Money/tax snapshots, jurisdiction, reconciliation, e-way and retention | [Issue #8 contract](money-tax-proof-privacy-contract.md) and [ADR 0009](../adr/0009-money-tax-proof-and-privacy-policy.md): exact paise, deterministic components, one final adjustment, W27 local policy, blocked unknown jurisdiction, immutable booking and separate e-way provenance | #8 policy; #21 tax, #29 payments, #30 receipts, #32/#67 e-way | #8 acceptance; current tax validation and finance mechanics before production | #20, #21, #22, #29, #30, #32, #62, #63, #67 | PARTIAL: #8 policy resolved on acceptance; [ADR 0019](../adr/0019-payment-ledger.md) resolves partial Booking-level collection, no negative credits, linked reversals and W20/W21; statutory outputs and downstream receipt/report implementation remain gated |
| D07 | Challenge lifetime, failures, resend/replacement and exceptional proof | [Issue #8 proof policy](money-tax-proof-privacy-contract.md#delivery-challenge-policy-for-issue-42), [secure deliveries](deliveries.md), [ADR 0030](../adr/0030-secure-atomic-delivery-proof.md): 10 minutes, 5 lineage failures, 60 seconds, 3 resends, no reset/reveal; W39 request/W40 independent responsible franchise_admin approval, W11 completion | #8 policy; #42 deliveries | Implemented for independent review; live Meta template qualification remains a deployment gate | #43, #45 | RESOLVED for doorstep implementation; deferred office lifecycle/calendar seam remains fail-closed |
...
| D09 | Private storage, revocation, quarantine, limits and retention | [ADR 0021](../adr/0021-private-attachment-storage.md): private S3-compatible protocol, clamd fail-closed, content detection, 8 MiB/file, 10 objects/32 MiB/3 pending per Booking, 15 min upload, 60 s authenticated grants and 5 min cleanup tick | #31 application boundary | Application design resolved before implementation | #33, #42, #68, #69, #72 | APPLICATION RESOLVED; hosted vendor/provisioning #68, backup/recovery #69, final legal periods/holds/deletion #72 remain downstream |
...
- #21/#29/#30: preserve #8 exact tax allocation and fixed Booking payable. ADR 0019 resolves partial Booking collection, no negative credits and linked reversals; #30 statutory output and receipt reconciliation remain downstream.
...
## D11 implementation decision — Issue #28

[ADR 0018](../adr/0018-atomic-route-events.md) bounds one atomic operational command to the frozen manifest maximum of 1,000 distinct Parcels, with organization/franchise/Route/sorted-Parcel lock order. API implementation decision is resolved for review; production contention/load qualification remains #74.

## Issue #29 financial decisions

[ADR 0019](../adr/0019-payment-ledger.md) resolves #29's portion of D06 for independent
review: partial collections against one immutable Booking gross; no Parcel allocation or
installment rounding; positive linked partial/full reversals only; no negative credits or
external refunds; W20/W21 franchise_admin F only; no manager role and no accountant mutation.
Financial and delivery corrections stay independent. Re-settlement uses a later ledger
revision, preserving the v1 payment.settled reference payload. Scoped ledger-derived
projection supplies #30/#61/#63 reconciliation. The implementation is documented in
[Payments](payments.md); it does not implement issued receipts, receipt correction, report
